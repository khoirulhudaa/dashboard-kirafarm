...
import { useLocation, useNavigate } from "react-router-dom"; // Tambah useNavigate
import { ThemeToggleButton } from "../components/common/ThemeToggleButton";
import { UserCircle2, ChevronDown, LogOut, User } from "lucide-react"; // Tambah icon
import { useSidebar } from "../context/SidebarContext";

const formatPageTitle = (pathname: string): string => {
  let cleanPath = pathname.replace(/^\/|\/$/g, "");
  if (cleanPath === "") return "Halaman Utama";
...
const AppHeader: React.FC = () => {
  ...
  const handleLogout = () => {
    ...
    // Hapus item lain jika ada, misal refreshToken dll

    setIsDropdownOpen(false);
    navigate("/signin"); // Ganti dengan route login kamu
  };
      ...
      <div className="flex flex-col items-center justify-between grow lg:flex-row lg:px-6">
        <div className="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4">
          ...
          <div className="hidden lg:block">
            <h1 className="text-md font-normal text-gray-900 dark:text-white">
              <a href="/" className="text-blue-600">KIRAFARM</a> / {pageTitle}
            </h1>
          </div>
        </div>
